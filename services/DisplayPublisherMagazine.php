...
class DisplayPublisherMagazine{
   private $dbOpertionObj;
    ...
    function listMagazineThumbnailPublisher ($isPublisherAlive,$publisherId){
        $result=null;
        if($isPublisherAlive)
        {
        $result=$this->dbOpertionObj->listMagazineThumbnailPublisher ($publisherId);
       /* $length=  count($result);
...
$listMagazine=$displayPublisherMagazine->listMagazineThumbnailPublisher ($SessionObj->checkIssetSessionPublisherId(),$SessionObj->getSessionPublisherId());
