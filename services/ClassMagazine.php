...
class ClassMagazine {
    ...
    {
        return $this->dbOperationObj->ListMagazines($this->catId);
    }
    ...
    function getSubscriptionId($userid){
        ...
        $resultForThumbnail=$dbOperationThumbnail->listThumbnailForMagazine($this->catId);
       $length=count($resultForThumbnail);
       for($i=0;$i<$length;$i++){
           $this->originalMagsWithoutSubscriptionStatus[$i]['magazineThumbnail']=$resultForThumbnail[$i]['magazineThumbnail'];
       }
