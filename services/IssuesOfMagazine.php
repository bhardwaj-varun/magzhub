...
class IssuesOfMagazine{
    ...
    function getDifferentIssuesOfMagazine($magazineId, $userId){
       
      $result=$this->dboperationObj->getDifferentIssuesOfMagazine($magazineId, $userId);
      $length=  count($result);
      for($i=0;$i<$length;$i++)
      {
...
if(isset($_POST['MagazineId']))
{
    if($SessionObj->checkIssetSessionUserId())
    {
       ...
       $result= $Issues->getDifferentIssuesOfMagazine($Issues->getMagazineId(), $SessionObj->getSessionUserId());
